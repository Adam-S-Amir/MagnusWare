...
function $embed2(options) {
    var $embed2 = $("<div>");
    var embed2 = $embed2[0];

    var disable_delegate_pointerup = false;

    $embed2.focus_contents = function () {
        if (!embed2.contentWindow) {
            return;
        }
        if (embed2.contentDocument.hasFocus()) {
            return;
        }

        disable_delegate_pointerup = true;
        embed2.contentWindow.focus();
        setTimeout(function () {
            embed2.contentWindow.focus();
    ...
    var delegate_pointerup = function () {
        ...
        if (embed2.contentWindow && embed2.contentWindow.jQuery) {
            embed2.contentWindow.jQuery("body").trigger("pointerup");
        }
        if (embed2.contentWindow) {
            const event = new embed2.contentWindow.MouseEvent("mouseup", {
                button: 0
            });
            embed2.contentWindow.dispatchEvent(event);
            const event2 = new embed2.contentWindow.MouseEvent("mouseup", {
                button: 2
            });
            embed2.contentWindow.dispatchEvent(event2);
        }
    };
    ...
    $embed2.on("load", function () {
        ...
        var $contentWindow = $(embed2.contentWindow);
